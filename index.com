...
.witem label.wlabel{display:flex;align-items:center;gap:12px;padding:13px 14px;border:1.5px solid var(--brd);border-radius:var(--rad-sm);cursor:pointer;font-size:14px;font-weight:500;color:var(--txt2);background:var(--wh);transition:all .2s;user-select:none;}
...
.witem-detail{display:none;background:var(--gll);border:1.5px solid var(--g3);border-top:none;border-radius:0 0 var(--rad-sm) var(--rad-sm);padding:10px 14px 12px;margin-top:-4px;}
...
.price-box{background:linear-gradient(135deg,#0a5230 0%,#1a7a4a 100%);border-radius:var(--rad);padding:18px 20px;margin-bottom:14px;box-shadow:0 4px 24px rgba(10,82,48,.25);position:relative;overflow:hidden;}
...
.upzone{border:2.5px dashed var(--brd);border-radius:var(--rad);padding:30px 16px;text-align:center;cursor:pointer;background:var(--gll);position:relative;transition:all .22s;}
...
<script>
// ════════════════════════════════════════
const GAS_URL = "https://script.google.com/macros/s/AKfycbxOHwWm11nsTXVv6xXv0KesusfTdKXZzMpssa4upz4JbkLGkxgPvJ-gGLmXUHe3fQqz/exec";
const NOTIFY_EMAIL = "mitrpholpkbwt@gmail.com";
// ════════════════════════════════════════
...
function buildGrid(){
  el('wgrid').innerHTML = WASTE_TYPES.map(w => `
    <div class="witem">
      ...
      <label class="wlabel" for="${w.id}">
        ...
        <span class="wprice-tag">${fp(w.price)} บ./กก.</span>
      </label>
...
function fp(n){ return n % 1 === 0 ? n.toString() : n.toFixed(1); }

function toggleDetail(id){
  const cb = el(id), det = el('det-'+id);
  if(cb.checked){ det.classList.add('show'); el('wt-'+id).focus(); }
  else { det.classList.remove('show'); el('wt-'+id).value = ''; }
  calcPrice();
}

function calcPrice(){
  let cur = 0; const lines = [];
  WASTE_TYPES.forEach(w => {
    const cb = el(w.id);
    if(!cb || !cb.checked) return;
    const wt = parseFloat(el('wt-'+w.id).value) || 0;
    const sub = wt * w.price; cur += sub;
  ...
  const re = el('pb-rows');
  if(!lines.length){ re.innerHTML = '<div class="pb-empty">เลือกประเภทขยะและกรอกน้ำหนักเพื่อดูราคา</div>'; return; }
  re.innerHTML = lines.map(l =>
...
function getTW(){ return WASTE_TYPES.reduce((s,w)=>{ const cb=el(w.id); if(!cb||!cb.checked) return s; return s+(parseFloat(el('wt-'+w.id).value)||0); },0); }
function getTA(){ return WASTE_TYPES.reduce((s,w)=>{ const cb=el(w.id); if(!cb||!cb.checked) return s; const wt=parseFloat(el('wt-'+w.id).value)||0; return s+wt*w.price; },0); }
function getIT(){ return WASTE_TYPES.filter(w=>el(w.id)&&el(w.id).checked).map(w=>{ const wt=parseFloat(el('wt-'+w.id).value)||0; return `${w.value} ${wt} กก. = ${(wt*w.price).toFixed(2)} บ.`; }).join(' | '); }
function getSum(){ return WASTE_TYPES.filter(w=>el(w.id)&&el(w.id).checked).map(w=>({ name:w.name,emoji:w.emoji,wt:parseFloat(el('wt-'+w.id).value)||0,price:w.price,sub:(parseFloat(el('wt-'+w.id).value)||0)*w.price })); }

function setFlow(n){
  [1,2,3].forEach(i=>{ el('fd'+i).className='flow-dot'+(i<n?' done':i===n?' on':''); el('fl'+i).className='flow-lbl'+(i<n?' done':i===n?' on':''); });
  [1,2].forEach(i=>{ el('fa'+i).className='flow-arr'+(i<n?' done':''); });
  el('flow').style.display = n > 3 ? 'none' : 'flex';
}
function goP2(){ if(!val()) return; renderSum(); show('p2'); setFlow(2); scrollTop(); }
function goP1(){ show('p1'); setFlow(1); scrollTop(); }
function show(id){ document.querySelectorAll('.page').forEach(p=>p.classList.remove('on')); el(id).classList.add('on'); }
function scrollTop(){ window.scrollTo({top:0,behavior:'smooth'}); }

function val(){
  const items = getSum();
  if(!items.length){ alert('กรุณาเลือกประเภทขยะอย่างน้อย 1 รายการค่ะ'); return false; }
  const nw = items.filter(i=>i.wt<=0);
  if(nw.length){ alert('กรุณาระบุน้ำหนักของ: '+nw.map(i=>i.name).join(', ')+' ค่ะ'); return false; }
  return true;
}
...
function renderSum(){
  const items = getSum(), total = getTA(), tw = getTW();
  let h = [['👤 ชื่อผู้ฝาก', g('u-name')||'-'], ['📷 รูปที่แนบ', photos.length+' รูป']]
    .map(([k,v]) => `<div class="srow"><span class="sk">${k}</span><span class="sv">${esc(v)}</span></div>`).join('');
  h += items.map(i =>
    `<div class="srow">
       <span class="sk">${i.emoji} ${i.name}<br><small style="color:var(--txt3);font-weight:400">${i.wt} กก. × ${fp(i.price)} บ./กก.</small></span>
       <span class="sv">${i.sub.toFixed(2)} บาท</span>
     </div>`).join('');
  h += `<div class="sum-total"><span class="sum-total-lbl">💰 รวมทั้งหมด (${tw.toFixed(2)} กก.)</span><span class="sum-total-val">${total.toFixed(2)} บาท</span></div>`;
  el('sumbox').innerHTML = h;
}
...
async function submit(){
  ...
  try{
    ...
    let dh = [['👤 ชื่อผู้ฝาก', payload.name||'-']]
      .map(([k,v]) => `<div class="srow"><span class="sk">${k}</span><span class="sv">${esc(v)}</span></div>`).join('');
    dh += items.map(i =>
      `<div class="srow">
         <span class="sk">${i.emoji} ${i.name}<br><small style="color:var(--txt3);font-weight:400">${i.wt} กก. × ${fp(i.price)} บ./กก.</small></span>
    ...
    dh += `<div class="sum-total"><span class="sum-total-lbl">📷 รูปที่ส่ง</span><span class="sum-total-val" style="font-size:16px">${photos.length} รูป</span></div>`;
    el('done-sum').innerHTML = dh;
...
function reset(){
  const sn = g('u-name');
  WASTE_TYPES.forEach(w=>{
    const cb=el(w.id); if(cb) cb.checked=false;
    const d=el('det-'+w.id); if(d) d.classList.remove('show');
    const wt=el('wt-'+w.id); if(wt) wt.value='';
  });
  el('u-name').value = sn;
  el('sbtn').disabled = false;
  photos = []; renderPrev(); calcPrice();
